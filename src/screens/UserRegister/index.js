...
const UserRegister = props => {
  ...
  const createUser = async () => {
    try {
      ...
      await firestore()
        ...
        .doc(user.uid)
        .set({
          name: tab == 1 ? name.trim() + ' ' + lastName.trim() : '',
          email: email.trim(),
      ...
      if (user) {
        ...
        Util.showMessage(
          ...
          'A verification link has been sent to your email.(Check your spam in case the email is not in your inbox.',
        );
  ...
  const sendNewProEmailAdmin = async (name, email) => {
    try {
      ...
      body.append(
        'body',
        `<h1>Hello Admin</h1><p>A New Professional ${name}(${email}) has just registered with us.</p>`,
      );
      await axios({
        method: 'post',
  ...
  const sendUnderReviewEmail = async (name, email) => {
    ...
    try {
      ...
      body.append(
        'body',
        `<h1>Hey ${name}</h1><p>Your account is under review, we will let you know once it is approved.</p>`,
      );
      await axios({
        method: 'post',
        url: 'https://softment.in/universal/mailer/sendmail.php',
            ...
            auth()
              ...
              .then(() => {
                Util.showMessage(
                  'error',
                  'Oops!',
                  'This account has been marked as deleted, please register with new email.',
                );
              });
          } else {
              ...
              auth()
                ...
                .then(() => {
                  ...
                  Util.showMessage(
                    'error',
                    'Oops!',
                    `This account is already associated with ${
                      user.data().userType
                    }.`,
                  );
                })
    ...
    >
      ...
      <View style={styles.container}>
        <View style={styles.loginBack}>
          ...
          />
          <View style={styles.borderViewTop} />
          <Text style={styles.login}>{`Create new \naccount`}</Text>
        </View>
