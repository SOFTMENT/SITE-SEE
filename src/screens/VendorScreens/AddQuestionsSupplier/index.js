import { Button, Center, Checkbox, HStack, Icon, IconButton, Input, ScrollView, Text, Toast, View } from "native-base"
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.mainView}>
                <HStack mb={1} alignItems={"center"} justifyContent={"space-between"}>
                    <Text fontSize={18} fontFamily={fonts.semiBold}>Questions</Text>
                    <Checkbox colorScheme={"danger"} isChecked={autoChatMode == "questions"} onChange={(isSelected)=>toggleCheckbox(isSelected,"questions")}/>
                </HStack>
                {
                    questions.length?questions.map(question=>{
                        return(
                            <HStack key={question.id} style={styles.card} alignItems={"center"}justifyContent={"space-between"}>
                                <Text fontSize={16}>{question.title}</Text>
                                <IconButton bgColor={colors.appDefaultColor} borderRadius={30} p={2} onPress={()=>{
                ...
                <Text my={5} textAlign={"center"}>-----OR----</Text>
                <HStack alignItems={"center"} justifyContent={"space-between"}>
                    <Text fontSize={18} fontFamily={fonts.semiBold}>Response</Text>
                    <Checkbox colorScheme={"danger"} isChecked={autoChatMode == "response"} onChange={(isSelected)=>toggleCheckbox(isSelected,"response")} />
                </HStack>
                <Input 
                    value={autoResponse}
                    onChangeText={(txt)=>setAutoResponse(txt)}
                    fontSize={14} mt={2} w="100%" py="0" 
                    InputRightElement={
                    <Button 
                        isLoading={updateLoader}
                        onPress={handleAutoResponse} 
                        size="xs" rounded="none" w="1/6" h="full" bgColor={colors.appDefaultColor}>
                    Update
                    </Button>} 
                />
            </View>
