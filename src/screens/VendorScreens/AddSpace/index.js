...
export default function AddSpace(props) {
    ...
    const handleAddSpace = async () => {
        if (!image) {
            ...
            try {
                ...
                navigation.goBack()
                Util.showMessage("success", "Success", "Space added successfully")
            } catch (error) {
                console.log(error)
            }
        }
    ...
    return (
        <KeyboardAwareScrollView style={styles.container} nestedScrollEnabled={false} bounces={false} keyboardShouldPersistTaps={"handled"}>
            <Header navigation={navigation} title="Add Space" back />
            ...
            <PhotoPicker
                ...
                isCover={true}
            />
            <LoaderComponent visible={loaderVisibility} title={"Uploading pictures"} />
            <LocationSelector
